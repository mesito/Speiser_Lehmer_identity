#!/usr/bin/env python3
"""
verify_all.py — independent recomputation of every numerical claim of the paper
'Tight zero pairs of the Riemann zeta function at height 8.4e9: a Speiser--Lehmer
identity, a displacement law, Euler mediation of the floor, and an X-free
certificate' (M. Ismail, 2026). Test groups T1--T22 map to Sections 2--9 and
Appendices A--C; see README.md.
...
Section 9 additions (T16-T22, resolution of Open Problem 8.5; all in the
default run, adding ~30 s; primes <= 1e7 sieved once in T6X and reused):
  T16 k-tail truncation constant Delta_0 (Lemma 9.1, Thm 9.9(a)): value,
      p=2 term, X-independence over 1e2..1e7, all-prime tail estimate
      (log-weighted integral bound, closed form).
  T17 mean-value identity (Lemma 9.3): Monte Carlo E[exp(-L_X)] at X=100,
      s=1, 3e5 samples on [0,2e6], rng=default_rng(19), vs prod M_p(-1)
      (midpoint rule N=4096).
  T18 effective second-moment constants (Lemma 9.4): sum a_p, Var(L_X),
      sigma, the 4X/H error bound and the mean bound at H=T^2.
  T19 certified hybrid Chernoff table (Thm 9.5): exact midpoint-rule
  ...
  T20 Lipschitz constant Lambda_1 (Lemma 9.7) + uniform union bound
      (Thm 9.8): N*d(u-m) at H=T=8.436e9, m=0.5, s optimized on a grid.
      NOTE: with the STRICT k<=3 integrand, N*d(8.86)=6.822e-2 and
      N*d(9.5)=1.905e-4 exceed the preprint's 2-sig-fig table entries
      (6.8e-2, 1.9e-4) by ~0.3% (the table values are the 2-sf rounding;
      the full-k variant of Thm 9.5 gives bounds 2-7% smaller, cf. sec 9.3).
      Asserted against the rounding-aware bounds 6.85e-2 / 1.95e-4.
  T21 Bahadur-Rao prefactor validation (Prop 9.6): product law over
  ...
  T22 per-pair certificate margin (Cor 9.2 + Prop 8.4, from
      ...
      eligible pairs); the preprint's 6.9-8.5 range describes the
...
lam14 = 1.0 - 0.113 ** 2 / varW          # sigma_U = sup bound 0.113 (sec05)
...
# ================================================================= T16 Delta_0
print("\n--- T16: k-tail truncation constant Delta_0 (Lemma 9.1, Thm 9.9(a)) ---")
x16 = P7 ** -0.5
...
# ================================================================= T17 mean value
print("\n--- T17: mean-value identity MC, X=100, s=1 (Lemma 9.3) ---")
NQ9 = 4096
...
# ================================================================= T18 moments
print("\n--- T18: effective second moment constants (Lemma 9.4) ---")
...
# ================================================================= T19 Chernoff
print("\n--- T19: certified hybrid Chernoff bounds (Thm 9.5) ---")
PR19 = P7[PR7 <= 1000]
...
# ================================================================= T20 uniform
print("\n--- T20: Lipschitz bound + uniform union bound (Lemma 9.7, Thm 9.8) ---")
...
# ================================================================= T21 Bahadur-Rao
print("\n--- T21: Bahadur-Rao prefactor, tilted IS (Prop 9.6) ---")
PR21 = P7[PR7 <= 10 ** 4]
npr21 = len(PR21)
...
# ================================================================= T22 certificate
print("\n--- T22: per-pair certificate margin (Cor 9.2 + Prop 8.4) ---")
