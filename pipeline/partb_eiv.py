...
(i) Classical attenuation: b_hat = b_obs / lam, lam = 1 - sU^2/Var(W),
    with sU^2 from the truncation-error model:
      - sup-norm bound (sec05): |U| <= (1/4) P(2) = 0.113  -> sU = 0.113
...
sup_bound = 0.25 * P2                       # sec05: (1/4) sum_p p^-2 ~= 0.11
sU2_rand = 0.0
Pk = {}
for k in range(4, 30):
...
lines.append('- sup-norm bound (sec05 rationale): |U| <= (1/4)*sum_p p^-2 = %.4f; '
...
lines.append('- Under the classical EIV model (proxy = truth + independent error of '
             'scale given by the sec05 sup bound), the attenuation factor is '
